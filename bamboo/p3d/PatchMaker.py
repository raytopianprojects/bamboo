...
class PatchMaker:
        ...
        def getFile(self):
            ...
            for patchfile, pv in plan:
                ...
                patchFilename = Filename(
                    patchfile.package.packageDir, patchfile.file.filename)
    ...
    class Package:
        ...
        def __init__(self, packageDesc, patchMaker, xpackage=None):
            self.packageDir = Filename(
                patchMaker.installDir, packageDesc.getDirname())
        ...
        def readDescFile(self, doProcessing=False):
            ...
            packageDescFullpath = Filename(
                self.patchMaker.installDir, self.packageDesc)
            ...
            if xarchive:
                ...
                if doProcessing and self.compressedFilename:
                    fromPathname = Filename(
                        self.packageDir, self.compressedFilename)
                    toPathname = Filename(
                        self.packageDir, self.baseFile.filename + '.pz')
        ...
        def writeDescFile(self):
            ...
            importDescFullpath = Filename(
                self.patchMaker.installDir, importDescFilename)
    ...
    def processPackage(self, package):
        ...
        if topPv != currentPv:
            ...
            filename = Filename(package.currentFile.filename +
                                '.%s.patch' % (package.patchVersion))
