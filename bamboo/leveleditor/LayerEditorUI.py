...
class LayerEditorUI(wx.Panel):
    def __init__(self, parent, editor):
        ...
        self.llist = wx.ListCtrl(
            self, -1, style=wx.LC_REPORT | wx.LC_EDIT_LABELS | wx.LC_NO_HEADER)
        ...
        if obj is not None:
            ...
            for j in range(len(layersData)):
                if layersData[j] == obj[OG.OBJ_UID]:
                    wx.MessageBox("Selected object already is this layer",
                                  self.editorTxt, wx.OK | wx.ICON_EXCLAMATION)
                    return
    ...
    def onShowMembers(self, event):
        ...
        dialog = wx.SingleChoiceDialog(
            None, layerName, self.editorTxt, layerMembers)
    ...
    def traverse(self):
        ...
        for index in range(self.llist.GetItemCount()):
            self.saveData.append("    ui.layerEditorUI.addLayerEntry('%s', %s )" % (
                self.llist.GetItemText(index), self.llist.GetItemData(index)))
        layersDataDictKeys = list(self.layersDataDict.keys())
        for i in range(len(layersDataDictKeys)):
            layerData = self.layersDataDict[layersDataDictKeys[i]]
            for j in range(len(layerData)):
                self.saveData.append("    ui.layerEditorUI.addLayerData(%s, '%s')" % (
                    layersDataDictKeys[i], layerData[j]))
