...
class SceneGraphUIBase(wx.Panel):
    ...
    def traversePandaObjects(self, parent, objNodePath):
        ...
        for child in objNodePath.getChildren():
            if child.hasTag('OBJRoot'):
                ...
                continue
            namestr = "%s.%s" % (child.node().getType(),
                                 child.node().getName())
    ...
    def changeHierarchy(self, data, x, y):
        ...
        if dragToItem.IsOk():
            ...
            # undo function setup...
            action = ActionChangeHierarchy(self.editor, self.tree.GetItemPyData(self.tree.GetItemParent(
                item)), self.tree.GetItemPyData(item), self.tree.GetItemPyData(dragToItem), data)
    ...
    def changeLabel(self, itemId, newName):
        ...
        if item:
            ...
            namestr = "%s_%s_%s" % (
                obj[OG.OBJ_DEF].name, newName, obj[OG.OBJ_UID])
    ...
    def populateExtraMenu(self):
        ...
        raise NotImplementedError(
            'populateExtraMenu() must be implemented in subclass')
    ...
    def onRename(self, evt=None):
        ...
        dialog = wx.TextEntryDialog(
            None, '', 'Input new name', defaultValue=self.currObj[OG.OBJ_NP].getName())
