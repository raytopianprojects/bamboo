...
class CurveEditor(DirectObject):
                ...
                if self.selected != None:
                    for item in self.curveControl:
                        if item[1] == self.selected:
                            ...
                            self.currentCurve.setVertex(
                                item[0], self.selected.getPos())
                            self.accept("DIRECT-delete",
                                        self.onControlerDelete)
                            return task.cont
    ...
    def updateScene(self):
        curveObjNP = self.editor.objectMgr.addNewCurve(
            self.curveControl, self.degree, nodePath=self.currentRope)
    ...
    def createControler(self, x, y):
        if self.view != None:
            ...
            iRay.collider.setFromLens(self.view.cam_node, x, y)
