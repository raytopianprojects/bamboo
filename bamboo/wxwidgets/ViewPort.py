...
class Viewport(WxPandaWindow, DirectObject):
    ...
    def initialize(self):
        self.Update()
        if self.win:
            self.cam2d = base.make_camera_2d(self.win)
            self.cam2d.node().setCameraMask(LE_CAM_MASKS[self.name])

        self.cam = base.cam_list[-1]
        ...
        self.bt = base.setup_mouse(self.win, True)
...
# self.accept("page_up-repeat", self.zoomIn)
    ...
    @staticmethod
    def makeLeft(parent): return Viewport.makeOrthographic(
        parent, 'left', Point3(600, 0, 0))

    @staticmethod
    def makeFront(parent): return Viewport.makeOrthographic(
        parent, 'front', Point3(0, -600, 0))

    @staticmethod
    def makeTop(parent): return Viewport.makeOrthographic(
        parent, 'top', Point3(0, 0, 600))
