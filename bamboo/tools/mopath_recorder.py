...
from bamboo.directtools.DirectGeometry import *
from bamboo.directtools.DirectSelection import *
import Pmw
import os
import sys
...
from bamboo.tkwidgets import EntryScale
...
PRF_UTILITIES = [
    'lambda: base.bamboo.camera.lookAt(render)',
    'lambda: base.bamboo.camera.setZ(render, 0.0)',
...
class MopathRecorder(AppShell, DirectObject):
    ...
    def appInit(self):
        ...
        # Top level node path
        self.recorderNodePath = base.bamboo.group.attachNewNode(self.name)
        ...
        self.nodePathDict['camera'] = base.bamboo.camera
    ...
    def createInterface(self):
        ...
        widget = self.createButton(frame, 'Recording', 'Node Path:',
                                   ...
                                   lambda s=self: base.bamboo.select(
                                       s.nodePath),
                                   side=LEFT, expand=0)
        widget['relief'] = FLAT
    ...
    def selectedNodePathHook(self, nodePath):
        ...
        if nodePath.id() in self.playbackMarkerIds:
            base.bamboo.select(self.playbackMarker)
        elif nodePath.id() in self.tangentMarkerIds:
            base.bamboo.select(self.tangentMarker)
    ...
    def manipulateObjectStartHook(self):
        ...
        if base.bamboo.selected.last:
            if base.bamboo.selected.last.id() == self.playbackMarker.id():
                self.manipulandumId = self.playbackMarker.id()
            elif base.bamboo.selected.last.id() == self.tangentMarker.id():
    ...
    def onDestroy(self, event):
        ...
        base.bamboo.deselect(self.playbackMarker)
        base.bamboo.deselect(self.tangentMarker)
        # Remove tasks
