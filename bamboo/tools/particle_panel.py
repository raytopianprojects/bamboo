...
from bamboo.tkpanels import Placer
from bamboo.particles import ForceGroup
from bamboo.particles import Particles
from bamboo.particles import ParticleEffect
import Pmw
import os
import sys
...
class ParticlePanel(AppShell):
    ...
    def createInterface(self):
        ...
        self.effectsLabelMenu.add_command(
            ...
            command=lambda s=self: base.bamboo.select(s.particleEffect))
        self.effectsLabelMenu.add_command(
        ...
        self.getVariable('Geom Renderer', 'Fbuffer Op.').set(
        ...
        rendererSpriteInterpolationPage = rendererSpriteNotebook.add(
            'Interpolate')
##################################################################################
...
##################################################################################
        ...
        self.getVariable('Sprite Renderer', 'Incoming Op.').set(
            'OIncomingAlpha')
        self.createOptionMenu(p, 'Sprite Renderer',
                              'Fbuffer Op.',
                              'See ColorBlendAttrib.h for explanation',
        ...
        self.getVariable('Sprite Renderer', 'Fbuffer Op.').set(
            'OOneMinusIncomingAlpha')
    ...
    def updateEmitterWidgets(self):
        ...
        if isinstance(emitter, BoxEmitter):
            ...
            self.getWidget('Ring Emitter', 'Radius Spread').set(
                radiusSpread, 0)
    ...
    def updateRendererWidgets(self):
        ...
        if isinstance(renderer, LineParticleRenderer):
            ...
            self.getWidget('Sprite Renderer', 'Frame Rate').set(
                renderer.getAnimateFramesRate(), 0)
            self.getVariable('Sprite Renderer', 'Enable Animation').set(
                renderer.getAnimateFramesEnable())
            # Updates widgets with renderer data.
    ...
    def addRendererSpriteAnimationTexture(self):
        ...
        if ren.addTextureFromFile():
            animId = len(
                [x for x in self.rendererSpriteAnimationWidgetList if x and x.valid])
            anim = ren.getAnim(animId)

            frameNum = len(
                [x for x in self.rendererSpriteAnimationWidgetList if x])
    ...
    def setRendererColorBlendAttrib(self, rendererName, blendMethodStr, incomingOperandStr, fbufferOperandStr):
        self.particles.getRenderer().setColorBlendMode(getattr(ColorBlendAttrib, blendMethodStr),
                                                       getattr(
                                                           ColorBlendAttrib, incomingOperandStr),
    ...
    def setRendererSpriteColorBlendMethod(self, blendMethod):
        ...
        incomingOperandStr = self.getVariable(
            'Sprite Renderer', 'Incoming Op.').get()
        fbufferOperandStr = self.getVariable(
            'Sprite Renderer', 'Fbuffer Op.').get()

        self.setRendererColorBlendAttrib(
            'Sprite Renderer', blendMethodStr, incomingOperandStr, fbufferOperandStr)

    def setRendererSpriteColorBlendIncomingOperand(self, operand):
        blendMethodStr = self.getVariable(
            'Sprite Renderer', 'Color Blend').get()
    ...
    def setRendererGeomColorBlendMethod(self, blendMethod):
        blendMethodStr = blendMethod
        incomingOperandStr = self.getVariable(
            'Geom Renderer', 'Incoming Op.').get()
        fbufferOperandStr = self.getVariable(
            'Geom Renderer', 'Fbuffer Op.').get()
        self.setRendererColorBlendAttrib(
            'Geom Renderer', blendMethodStr, incomingOperandStr, fbufferOperandStr)
    ...
    def createSpriteAnimationTextureWidget(self, parent, anim, animName):
        ...
        Label(f, text='Texture: ', font=(
            'MSSansSerif', 12), width=7).pack(side=LEFT)
        strVar = StringVar()
        entry = Entry(f, textvariable=strVar).pack(
            padx=3, pady=3, side=LEFT, fill=X, expand=1)
        if frame.valid:
    ...
    def createSpriteAnimationNodeWidget(self, parent, anim, animName):
        ...
        Label(lf, text='Model: ', font=(
            'MSSansSerif', 12), width=7).pack(side=LEFT)
        mStrVar = StringVar()
        entry = Entry(lf, textvariable=mStrVar).pack(
            padx=3, pady=3, side=LEFT, fill=X, expand=1)
        if frame.valid:
        ...
        lf = Frame(f)
        lf.pack(fill=X, expand=1)
        Label(lf, text='Node: ', font=(
            'MSSansSerif', 12), width=7).pack(side=LEFT)
        nStrVar = StringVar()
        entry = Entry(lf, textvariable=nStrVar).pack(
            padx=3, pady=3, side=LEFT, fill=X, expand=1)
        if frame.valid:
    ...
    def readSpriteRendererAnimations(self):
        ...
        for anim in [ren.getAnim(x) for x in range(ren.getNumAnims())]:
            if (anim.getSourceType() == SpriteAnim.STTexture):
                w = self.createSpriteAnimationTextureWidget(
                    self.rendererSpriteAnimationFrame, anim, repr(len(self.rendererSpriteAnimationWidgetList)))
            else:
                w = self.createSpriteAnimationNodeWidget(self.rendererSpriteAnimationFrame, anim, repr(
                    len(self.rendererSpriteAnimationWidgetList)))
        ...
        for x in range(len(self.rendererSpriteAnimationWidgetList)):
            ...
            if (widget and widget.valid):
                if (widget.animSourceType == SpriteAnim.STTexture):
                    texSource = self.getVariable(
                        'Sprite Renderer', repr(x) + ' Anim Texture').get()
                    ren.addTextureFromFile(texSource)
                else:
                    modelSource = self.getVariable(
                        'Sprite Renderer', repr(x) + ' Anim Model').get()
                    nodeSource = self.getVariable(
                        'Sprite Renderer', repr(x) + ' Anim Node').get()
