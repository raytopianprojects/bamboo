...
from .direct_globals import DIRECT_NO_MOD
from .direct_camera_control import DirectCameraControl
from .direct_manipulation import DirectManipulationControl
from .direct_selection import SelectionRay, COA_ORIGIN, SelectedNodePaths
from .direct_grid import DirectGrid
# from DirectGeometry import *
from .direct_lights import DirectLights
from bamboo.cluster.ClusterClient import createClusterClient, DummyClusterClient
from bamboo.cluster.ClusterServer import ClusterServer
...
class DirectSession(DirectObject):
    ...
    def __init__(self):
        ...
        self.grid.disable()
        self.lights = DirectLights(base.bamboo.group)
        ...
        if base.config.GetBool('want-vrpn', 0):
            from bamboo.directdevices import DirectDeviceManager
            ...
            if joybox:
                from bamboo.directdevices import DirectJoybox
                self.joybox = DirectJoybox.DirectJoybox(joybox)
            if radamec:
                from bamboo.directdevices import DirectRadamec
                self.radamec = DirectRadamec.DirectRadamec(radamec)
            if fastrak:
                from bamboo.directdevices import DirectFastrak
        ...
        if base.want_tk:
            from bamboo.showbase import TkGlobal
            from bamboo.tkpanels import DirectSessionPanel
    ...
    def oobe(self):
        ...
        if self.oobeMode:
            ...
            base.bamboo.cameraControl.camManipRef.setPosHpr(
                self.trueCamera, 0, 0, 0, 0, 0, 0)
            ival = self.oobeCamera.posHprInterval(
                2.0, pos=Point3(0), hpr=Vec3(0),
                other=base.bamboo.cameraControl.camManipRef,
                blendType='easeInOut')
            ...
            # Position a target point to lerp the oobe camera to
            base.bamboo.cameraControl.camManipRef.setPos(
                self.trueCamera, Vec3(-2, -20, 5))
            base.bamboo.cameraControl.camManipRef.lookAt(self.trueCamera)
            ival = self.oobeCamera.posHprInterval(
                2.0, pos=Point3(0), hpr=Vec3(0),
                other=base.bamboo.cameraControl.camManipRef,
                blendType='easeInOut')
    ...
    def beginOOBE(self):
        ...
        self.oobeCamera.setPosHpr(
            base.bamboo.cameraControl.camManipRef, 0, 0, 0, 0, 0, 0)
        base.bamboo.camera = self.oobeCamera
        self.oobeMode = 1
    ...
    def endOOBE(self):
        ...
        base.bamboo.camera = self.trueCamera
    ...
    def inputHandler(self, input):
        if not hasattr(self, 'oobeMode') or self.oobeMode == 0:
            ...
            if base.bamboo.manipulationControl.fMultiView:
                ...
                if (self.fMouse1 or self.fMouse2 or self.fMouse3) and\
                   input[4:7] != base.bamboo.camera.getName()[:3] and\
                   input.endswith('-up'):
                    # to handle orphan events
                    return
                ...
                if input not in self.modifierEvents:
                    ...
                    if hasattr(winCtrl, 'grid'):
                        base.bamboo.grid = winCtrl.grid
                    base.bamboo.dr = base.bamboo.drList[base.cam_list.index(
                        NodePath(winCtrl.camNode))]
                    base.bamboo.iRay = base.bamboo.dr.iRay
                    base.mouseWatcher = winCtrl.mouseWatcher
                    base.mouseWatcherNode = winCtrl.mouseWatcher.node()
                    base.bamboo.dr.mouseUpdate()
                    LE_showInOneCam(self.selectedNPReadout,
                                    self.camera.getName())
                    base.bamboo.widget = base.bamboo.manipulationControl.widgetList[base.cam_list.index(
                        NodePath(winCtrl.camNode))]
        ...
        if not taskMgr.hasTaskNamed('resizeObjectHandles'):
            ...
            if dnp:
                ...
                if self.manipulationControl.fMultiView:
                    for i in range(3):
                        sf = 30.0 * bamboo.drList[i].orthoFactor
                    ...
                    sf = 0.075 * nodeCamDist * \
                        math.tan(deg2Rad(bamboo.drList[3].fovV))
                    self.manipulationControl.widgetList[3].setDirectScalingFactor(
                        sf)

                else:
                    nodeCamDist = Vec3(dnp.getPos(bamboo.camera)).length()
                    sf = 0.075 * nodeCamDist * \
                        math.tan(deg2Rad(bamboo.drList.getCurrentDr().fovV))
    ...
    def isNotCycle(self, nodePath, parent):
        if nodePath == parent:
            print('bamboo.reparent: Invalid parent')
            return 0
    ...
    def useObjectHandles(self):
        ...
        self.widget.reparentTo(base.bamboo.group)
...
class DisplayRegionContext(DirectObject):
    ...
    def getWidth(self):
        prop = base.bamboo.win.getProperties()
        if prop.hasSize():
...
class DisplayRegionList(DirectObject):
    ...
    def mouseUpdate(self, modifiers=DIRECT_NO_MOD):
        for dr in self.displayRegionList:
            dr.mouseUpdate()
        # base.bamboo.dr = self.getCurrentDr()

    def getCurrentDr(self):
        if not self.tryToGetCurrentDr:
            return base.bamboo.dr
        for dr in self.displayRegionList:
