...
from bamboo.tkwidgets.AppShell import *
from panda3d.core import *
import Pmw
from bamboo.tkwidgets import Dial
from bamboo.tkwidgets import Floater
from bamboo.tkwidgets import Slider
from bamboo.tkwidgets import VectorWidgets
from bamboo.tkwidgets import SceneGraphExplorer
from .task_manager_panel import TaskManagerWidget
from bamboo.tkwidgets import MemoryExplorer
...
class DirectSessionPanel(AppShell):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Active light
        if len(base.bamboo.lights) > 0:
            name = base.bamboo.lights.getNameList()[0]
    ...
    def appInit(self):
        ...
        self.nodePathDict = {}
        self.nodePathDict['widget'] = base.bamboo.widget
        ...
        self.jbNodePathDict['widget'] = base.bamboo.widget
        self.jbNodePathDict['camera'] = base.bamboo.camera
    ...
    def createInterface(self):
        ...
        self.directGridEnabled.set(base.bamboo.grid.isEnabled())
        self.menuBar.addmenuitem('DIRECT', 'checkbutton',
                                 'DIRECT Grid Enabled',
                                 label='Enable Grid',
        ...
        self.menuBar.addmenuitem('DIRECT', 'command',
                                 ...
                                 command=base.bamboo.toggleWidgetVis)

        self.menuBar.addmenuitem(
            'DIRECT', 'command',
            'Toggle Widget Move/COA Mode',
            label='Toggle Widget Mode',
            command=base.bamboo.manipulationControl.toggleObjectHandlesMode)
        ...
        self.menuBar.addmenuitem('DIRECT', 'command',
                                 'Deselect All',
                                 label='Deselect All',
                                 command=base.bamboo.deselectAll)
        ...
        self.undoButton = Button(menuFrame, text='Undo',
                                 command=base.bamboo.undo)
        if base.bamboo.undoList:
        ...
        self.redoButton = Button(menuFrame, text='Redo',
                                 command=base.bamboo.redo)
        if base.bamboo.redoList:
    ...
    def createEnvPage(self, envPage):
        ...
        nameList = ['Display Region ' + repr(x)
                    for x in range(len(base.bamboo.drList))]
        ...
        self.toggleLightsButton = Button(
            toggleFrame,
            text='Lights',
            command=base.bamboo.lights.toggle)
    ...
    def createLightsPage(self, lightsPage):
        ...
        nameList = base.bamboo.lights.getNameList()
    ...
    def createGridPage(self, gridPage):
        ...
        self.gridSpacing = Floater.Floater(
            ...
            text='Grid Spacing',
            min=0.1,
            value=base.bamboo.grid.getGridSpacing())
        self.gridSpacing['command'] = base.bamboo.grid.setGridSpacing
        ...
        self.gridSize = Floater.Floater(
            ...
            text='Grid Size',
            min=1.0,
            value=base.bamboo.grid.getGridSize())
        self.gridSize['command'] = base.bamboo.grid.setGridSize
        ...
        self.gridSnapAngle = Dial.AngleDial(
            ...
            text='Snap Angle',
            style='mini',
            value=base.bamboo.grid.getSnapAngle())
        self.gridSnapAngle['command'] = base.bamboo.grid.setSnapAngle
    ...
    def createDevicePage(self, devicePage):
        ...
        if base.bamboo.joybox != None:
            ...
            self.jbXyzSF['command'] = (
                lambda v: base.bamboo.joybox.setXyzMultiplier(v))
            ...
            self.jbHprSF['command'] = (
                lambda v: base.bamboo.joybox.setHprMultiplier(v))
    ...
    def toggleDirectGrid(self):
        if self.directGridEnabled.get():
            base.bamboo.grid.enable()
        else:
            base.bamboo.grid.disable()

    def toggleWidgetOnTop(self):
        if self.directWidgetOnTop.get():
            base.bamboo.widget.setBin('gui-popup', 0)
            base.bamboo.widget.setDepthTest(0)
        else:
            base.bamboo.widget.clearBin()
            base.bamboo.widget.setDepthTest(1)
    ...
    def selectJBModeNamed(self, name):
        if name == 'Joe Mode':
            base.bamboo.joybox.joeMode()
        elif name == 'Drive Mode':
            base.bamboo.joybox.driveMode()
        elif name == 'Orbit Mode':
            base.bamboo.joybox.orbitMode()
        elif name == 'Look At Mode':
            base.bamboo.joybox.lookAtMode()
        elif name == 'Look Around Mode':
            base.bamboo.joybox.lookAroundMode()
        elif name == 'Walkthru Mode':
            base.bamboo.joybox.walkthruMode()
        elif name == 'Demo Mode':
            base.bamboo.joybox.demoMode()
        elif name == 'HPRXYZ Mode':
            base.bamboo.joybox.hprXyzMode()

    def selectJBNodePathNamed(self, name):
        if name == 'selected':
            nodePath = base.bamboo.selected.last
        ...
        if (nodePath != None):
            ...
            if (nodePath == 'No Node Path'):
                base.bamboo.joybox.setNodePath(None)
            else:
                base.bamboo.joybox.setNodePath(nodePath)
    ...
    def selectDisplayRegionNamed(self, name):
        if name.find('Display Region ') >= 0:
            drIndex = int(name[-1:])
            self.activeDisplayRegion = base.bamboo.drList[drIndex]
        else:
    ...
    def selectLightNamed(self, name):
        # See if light exists
        self.activeLight = base.bamboo.lights[name]
        # If not...create new one
        if self.activeLight == None:
            self.activeLight = base.bamboo.lights.create(name)
        ...
        if self.activeLight:
            ...
            listbox.setlist(base.bamboo.lights.getNameList())
            if len(base.bamboo.lights) > 0:
                self.lightMenu.selectitem(base.bamboo.lights.getNameList()[0])
        # Make sure info is current
        self.updateLightInfo()

    def addAmbient(self):
        return base.bamboo.lights.create('ambient')

    def addDirectional(self):
        return base.bamboo.lights.create('directional')

    def addPoint(self):
        return base.bamboo.lights.create('point')

    def addSpot(self):
        return base.bamboo.lights.create('spot')
    ...
    def toggleLights(self):
        if self.enableLights.get():
            base.bamboo.lights.allOn()
        else:
            base.bamboo.lights.allOff()

    def toggleActiveLight(self):
        if self.activeLight:
            if self.lightActive.get():
                base.bamboo.lights.setOn(self.activeLight)
            else:
                base.bamboo.lights.setOff(self.activeLight)
    ...
    def toggleXyzSnap(self):
        base.bamboo.grid.setXyzSnap(self.xyzSnap.get())

    def toggleHprSnap(self):
        base.bamboo.grid.setHprSnap(self.hprSnap.get())

    # DEVICE CONTROLS
    def toggleJoybox(self):
        if self.enableJoybox.get():
            base.bamboo.joybox.enable()
        else:
            base.bamboo.joybox.disable()
    ...
    def updateGridInfo(self):
        self.enableGrid.set(base.bamboo.grid.isEnabled())
        self.xyzSnap.set(base.bamboo.grid.getXyzSnap())
        self.hprSnap.set(base.bamboo.grid.getHprSnap())
        self.gridSpacing.set(base.bamboo.grid.getGridSpacing(), 0)
        self.gridSize.set(base.bamboo.grid.getGridSize(), 0)
        self.gridSnapAngle.set(base.bamboo.grid.getSnapAngle(), 0)

    # UNDO/REDO
    def pushUndo(self, fResetRedo=1):
        base.bamboo.pushUndo([self['nodePath']])
    ...
    def pushRedo(self):
        base.bamboo.pushRedo([self['nodePath']])
