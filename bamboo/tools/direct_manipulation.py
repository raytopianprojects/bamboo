...
class DirectManipulationControl(DirectObject):
    def __init__(self):
        ...
        # [gjeon] to be used in new LE's camera control
        ...
        self.initScaleMag = 1
        self.manipRef = base.bamboo.group.attachNewNode('manipRef')
        self.hitPtDist = 0
    ...
    def scaleWidget(self, factor):
        if hasattr(base.direct, 'widget'):
            base.bamboo.widget.multiplyScalingFactorBy(factor)
        else:
    ...
    def supportMultiView(self):
        ...
        self.widgetList = [self.topViewWidget, self.frontViewWidget,
                           self.leftViewWidget, self.objectHandles]
    ...
    def manipulationStart(self, modifiers):
        ...
        if base.bamboo.cameraControl.useMayaCamControls and modifiers == 4:
            self.mode = 'camera'
        ...
        if self.fScaling1D == 0 and\
           self.fScaling3D == 0:
            ...
            entry = base.bamboo.iRay.pickWidget(skipFlags=SKIP_WIDGET)
            # Did we hit a widget?
            if entry:
                # Yes!
                self.hitPt.assign(entry.getSurfacePoint(
                ...
                if base.bamboo.cameraControl.useMayaCamControls and not base.bamboo.gotControl(modifiers) \
                   and not self.fAllowMarquee:
                    return
        else:
            entry = None

        if not base.bamboo.gotAlt(modifiers):
            if entry:
                ...
                watchMouseTask.initX = base.bamboo.dr.mouseX
                watchMouseTask.initY = base.bamboo.dr.mouseY
                taskMgr.add(watchMouseTask, 'manip-watch-mouse')
            else:
                if base.bamboo.fControl:
                    self.mode = 'move'
                    self.manipulateObject()
                elif not base.bamboo.fAlt and self.fAllowMarquee:
                    self.moveDir = None
                    watchMarqueeTask = Task.Task(self.watchMarqueeTask)
                    watchMarqueeTask.initX = base.bamboo.dr.mouseX
                    watchMarqueeTask.initY = base.bamboo.dr.mouseY
    ...
    def watchMouseTask(self, state):
        if (((abs(state.initX - base.bamboo.dr.mouseX)) > 0.01) or
                ((abs(state.initY - base.bamboo.dr.mouseY)) > 0.01)):
    ...
    def drawMarquee(self, startX, startY):
        ...
        if base.bamboo.cameraControl.useMayaCamControls and base.bamboo.fAlt:
            return
        if base.bamboo.fControl:
            return

        endX = base.bamboo.dr.mouseX
        endY = base.bamboo.dr.mouseY

        if (((abs(endX - startX)) < 0.01) and
                ((abs(endY - startY)) < 0.01)):
            return

        self.marquee = LineNodePath(
            render2d, 'marquee', 0.5, VBase4(.8, .6, .6, 1))
        ...
        if self.fMultiView:
            LE_showInOneCam(self.marquee, base.bamboo.camera.getName())
    ...
    def manipulationStop(self):
        ...
        if self.mode == 'select':
            ...
            if self.marquee:
                ...
                lens = base.bamboo.cam.node().getLens()
                ...
                marqueeFrustum = BoundingHexahedron(
                    fll, flr, fur, ful, nll, nlr, nur, nul)
                marqueeFrustum.xform(
                    base.bamboo.cam.getNetTransform().getMat())
                ...
                for geom in render.findAllMatches("**/+GeomNode"):
                    if (skipFlags & SKIP_HIDDEN) and geom.isHidden():
                        # Skip if hidden node
                        continue
# elif (skipFlags & SKIP_BACKFACE) and base.bamboo.iRay.isEntryBackfacing():
...
# pass
                    ...
                    # Can pick unpickable, use the first visible node
                          ...
                          (geom.getName() in base.bamboo.iRay.unpickable)):
                ...
                for nodePath in selectionList:
                    base.bamboo.select(nodePath, 1)

            else:
                entry = base.bamboo.iRay.pickGeom(skipFlags=skipFlags)
                if entry:
                    ...
                    # Select it
                    base.bamboo.select(
                        entry.getIntoNodePath(), base.bamboo.fShift)
                else:
                    base.bamboo.deselectAll()
        # elif self.mode == 'move':
    ...
    def manipulateObjectCleanup(self):
        if self.fScaling3D or self.fScaling1D:
            ...
            if hasattr(base.direct, 'widget'):
                base.bamboo.widget.transferObjectHandlesScale()
            else:
                self.objectHandles.transferObjectHandlesScale()
            self.fScaling3D = 0
            self.fScaling1D = 0
        base.bamboo.selected.highlightAll()
        if hasattr(base.direct, 'widget'):
            base.bamboo.widget.showAllHandles()
        else:
            self.objectHandles.showAllHandles()
        if base.bamboo.clusterMode == 'client':
            cluster(
                'base.bamboo.manipulationControl.objectHandles.showAllHandles()')
        if hasattr(base.direct, 'widget'):
            base.bamboo.widget.hideGuides()
        else:
        ...
        messenger.send('DIRECT_manipulateObjectCleanup',
                       [base.bamboo.selected.getSelectedAsList()])

    def spawnFollowSelectedNodePathTask(self):
        # If nothing selected, just return
        if not base.bamboo.selected.last:
            return
        ...
        pos = VBase3(0)
        hpr = VBase3(0)
        decomposeMatrix(base.bamboo.selected.last.mCoa2Dnp,
        ...
        t.pos = pos
        t.hpr = hpr
        t.base = base.bamboo.selected.last
        # Spawn the task
        taskMgr.add(t, 'followSelectedNodePath')

    def followSelectedNodePathTask(self, state):
        if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
            ...
            if self.worldSpaceManip:
                ...
                base.bamboo.widget.setPosHpr(state.base, state.pos, state.hpr)
        return Task.cont
    ...
    def toggleObjectHandlesMode(self):
        if self.fMovable:
            ...
            if self.fSetCoa:
                if hasattr(base.direct, 'widget'):
                    base.bamboo.widget.coaModeColor()
                else:
                    self.objectHandles.coaModeColor()
            else:
                if hasattr(base.direct, 'widget'):
                    base.bamboo.widget.manipModeColor()
                else:
                    self.objectHandles.manipModeColor()
        else:
            if hasattr(base.direct, 'widget'):
                base.bamboo.widget.disabledModeColor()
            else:
    ...
    def manipulateObject(self):
        ...
        selectedList = base.bamboo.selected.getSelectedAsList()
        ...
        if selectedList:
            ...
            if hasattr(base.direct, 'widget'):
                base.bamboo.widget.showGuides()
                base.bamboo.widget.hideAllHandles()
                base.bamboo.widget.showHandle(self.constraint)
            else:
                self.objectHandles.showGuides()
                self.objectHandles.hideAllHandles()
                self.objectHandles.showHandle(self.constraint)
            if base.bamboo.clusterMode == 'client':
                oh = 'base.bamboo.manipulationControl.objectHandles'
            ...
            base.bamboo.selected.dehighlightAll()
    ...
    def spawnManipulateObjectTask(self):
        ...
        t.fMouseX = abs(base.bamboo.dr.mouseX) > 0.9
        t.fMouseY = abs(base.bamboo.dr.mouseY) > 0.9
        if t.fMouseX:
            t.constrainedDir = 'y'
        else:
            t.constrainedDir = 'x'
        # Compute widget's xy coords in screen space
        t.coaCenter = getScreenXY(base.bamboo.widget)
    ...
    def manipulateObjectTask(self, state):
        if self.fScaling1D:
            ...
            if self.constraint:
                ...
                if self.useSeparateScaleHandles:
                    ...
                    if base.bamboo.fControl and not self.currEditTypes & EDIT_TYPE_UNSCALABLE:
                        if type == 'post':
            ...
            # No widget interaction, determine free manip mode
                ...
                if 0 and (self.fScaling1D or self.fScaling3D) and (not base.bamboo.fAlt):
                    if hasattr(base.direct, 'widget'):
                        base.bamboo.widget.transferObjectHandleScale()
                    else:
                        self.objectHandles.transferObjectHandlesScale()
                ...
                # Otherwise, manip mode depends on where you started
                    ...
                    if base.bamboo.fShift or base.bamboo.fControl:
        ...
        if self.fSetCoa:
            ...
            base.bamboo.selected.last.mCoa2Dnp.assign(
                base.bamboo.widget.getMat(base.bamboo.selected.last))
        else:
    ...
    def gridSnapping(self, nodePath, offset):
        ...
        if offsetX < 0.0:
            ...
            signX = 1.0
        modX = math.fabs(offsetX) % base.bamboo.grid.gridSpacing
        floorX = math.floor(math.fabs(offsetX) / base.bamboo.grid.gridSpacing)
        if modX < base.bamboo.grid.gridSpacing / 2.0:
            offsetX = signX * floorX * base.bamboo.grid.gridSpacing
        else:
            offsetX = signX * (floorX + 1) * base.bamboo.grid.gridSpacing

        if offsetY < 0.0:
            signY = -1.0
        else:
            signY = 1.0
        modY = math.fabs(offsetY) % base.bamboo.grid.gridSpacing
        floorY = math.floor(math.fabs(offsetY) / base.bamboo.grid.gridSpacing)
        if modY < base.bamboo.grid.gridSpacing / 2.0:
            offsetY = signY * floorY * base.bamboo.grid.gridSpacing
        else:
            offsetY = signY * (floorY + 1) * base.bamboo.grid.gridSpacing

        if offsetZ < 0.0:
            signZ = -1.0
        else:
            signZ = 1.0
        modZ = math.fabs(offsetZ) % base.bamboo.grid.gridSpacing
        floorZ = math.floor(math.fabs(offsetZ) / base.bamboo.grid.gridSpacing)
        if modZ < base.bamboo.grid.gridSpacing / 2.0:
            offsetZ = signZ * floorZ * base.bamboo.grid.gridSpacing
        else:
            offsetZ = signZ * (floorZ + 1) * base.bamboo.grid.gridSpacing
    ...
    def xlate1D(self, state):
        ...
        if self.fHitInit:
            ...
            if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
                ...
                # if base.bamboo.camera.getName() != 'persp':
                    # self.prevHit.assign(self.hitPt)
            else:
                if self.fGridSnap:
                    base.bamboo.widget.setPos(
                        self.gridSnapping(base.bamboo.widget, offset))
                else:
                    base.bamboo.widget.setPos(base.bamboo.widget, offset)
    ...
    def xlate2D(self, state):
        ...
        self.hitPt.assign(self.objectHandles.getWidgetIntersectPt(
            base.bamboo.widget, self.constraint[:1]))
    ...
    def rotate1D(self, state):
        ...
        if self.fHitInit:
            ...
            self.rotationCenter = getScreenXY(base.bamboo.widget)
        ...
        if self.rotateAxis == 'x':
            if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
                ...
                base.bamboo.widget.setP(base.bamboo.widget, deltaAngle)
        elif self.rotateAxis == 'y':
            if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
                for widget in base.bamboo.manipulationControl.widgetList:
                    widget.setR(widget, deltaAngle)
            else:
                base.bamboo.widget.setR(base.bamboo.widget, deltaAngle)
        elif self.rotateAxis == 'z':
            if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
                for widget in base.bamboo.manipulationControl.widgetList:
                    widget.setH(widget, deltaAngle)
            else:
                base.bamboo.widget.setH(base.bamboo.widget, deltaAngle)
    ...
    def widgetCheck(self, type):
        ...
        mWidget2Cam = base.bamboo.widget.getMat(base.bamboo.camera)
    ...
    def xlateCamXZ(self, state):
        ...
        base.bamboo.widget.setX(
            base.bamboo.camera,
            x + 0.5 * dr.mouseDeltaX * dr.nearWidth * (y/dr.near))
        base.bamboo.widget.setZ(
            base.bamboo.camera,
    ...
    def xlateCamXY(self, state):
        ...
        if self.fHitInit:
            ...
            coaCenter = getNearProjectionPoint(base.bamboo.widget)
            self.deltaNearX = coaCenter[0] - base.bamboo.dr.nearVec[0]
        ...
        # Move widget
        base.bamboo.widget.setPos(base.bamboo.camera, vWidget2Camera)
    ...
    def rotate2D(self, state):
        ...
        relHpr(base.bamboo.widget, base.bamboo.camera, deltaX * tumbleRate,
               -deltaY * tumbleRate, 0)
    ...
    def rotateAboutViewVector(self, state):
        ...
        # Mouse motion edge to edge of display region results in one full turn
        relHpr(base.bamboo.widget, base.bamboo.camera, 0, 0, -deltaAngle)

    def scale1D(self, state):
        if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
            self.hitPtScale.assign(
                self.objectHandles.getAxisIntersectPt(self.constraint[:1]))
            self.hitPtScale = self.objectHandles.getMat().xformVec(self.hitPtScale)
            if self.fScaleInit1:
                # First time through just record hit point
                self.fScaleInit1 = 0
                self.prevHitScale.assign(self.hitPtScale)
                self.origScale = base.bamboo.widget.getScale()
            else:
                widgetPos = base.bamboo.widget.getPos()
                ...
                if self.constraint[:1] == 'x':
                    currScale = Vec3(currScale.getX() * d1/d0,
                                     currScale.getY(), currScale.getZ())
                elif self.constraint[:1] == 'y':
                    currScale = Vec3(
                        currScale.getX(), currScale.getY() * d1/d0, currScale.getZ())
                elif self.constraint[:1] == 'z':
                    currScale = Vec3(
                        currScale.getX(), currScale.getY(), currScale.getZ() * d1/d0)
                base.bamboo.widget.setScale(currScale)
            return
        ...
        if self.fScaleInit:
            ...
            self.initScaleMag = Vec3(
                self.objectHandles.getAxisIntersectPt(self.constraint[:1])).length()
            # record initial scale
            self.initScale = base.bamboo.widget.getScale()
        # Reset fHitInitFlag
        self.fHitInit = 1

        # reset the scale of the scaling widget so the calls to
        # getAxisIntersectPt calculate the correct distance
        base.bamboo.widget.setScale(1, 1, 1)

        # Scale factor is ratio current mag with init mag
        if self.constraint[:1] == 'x':
            currScale = Vec3(self.initScale.getX() *
                             self.objectHandles.getAxisIntersectPt(
                                 'x').length() / self.initScaleMag,
                             self.initScale.getY(), self.initScale.getZ())
        elif self.constraint[:1] == 'y':
            currScale = Vec3(self.initScale.getX(),
                             self.initScale.getY() * self.objectHandles.getAxisIntersectPt('y').length() /
                             self.initScaleMag,
                             self.initScale.getZ())
        elif self.constraint[:1] == 'z':
    ...
    def scale3D(self, state):
        if hasattr(base.direct, "manipulationControl") and base.bamboo.manipulationControl.fMultiView:
            if self.useSeparateScaleHandles:
                ...
                if self.fScaleInit1:
                    ...
                    # Scale factor is ratio current mag with init mag
                    currScale = Vec3(
                        currScale.getX() * d1/d0, currScale.getY() * d1/d0, currScale.getZ() * d1/d0)
                    base.bamboo.widget.setScale(currScale)
                return
            else:
                self.hitPtScale.assign(
                    self.objectHandles.getMouseIntersectPt())
        ...
        if self.fScaleInit:
            self.fScaleInit = 0
            self.manipRef.setPos(base.bamboo.widget, 0, 0, 0)
            self.manipRef.setHpr(base.bamboo.camera, 0, 0, 0)
            self.initScaleMag = Vec3(
    ...
    def plantSelectedNodePath(self):
        ...
        entry = base.bamboo.iRay.pickGeom(
            skipFlags=SKIP_HIDDEN | SKIP_BACKFACE | SKIP_CAMERA)
        # MRM: Need to handle moving COA
        if (entry != None) and (base.bamboo.selected.last != None):
            # Record undo point
            base.bamboo.pushUndo(base.bamboo.selected)
            # Record wrt matrix
            base.bamboo.selected.getWrtAll()
            # Move selected
            base.bamboo.widget.setPos(
                base.bamboo.camera, entry.getSurfacePoint(entry.getFromNodePath()))
...
class ObjectHandles(NodePath, DirectObject):
    ...
    def showWidgetIfActive(self):
        if self.fActive:
            self.reparentTo(base.bamboo.group)
    ...
    def enableHandle(self, handle):
        ...
        elif handle == 'x-scale' and base.bamboo.manipulationControl.useSeparateScaleHandles:
        ...
        elif handle == 'y-scale' and base.bamboo.manipulationControl.useSeparateScaleHandles:
        ...
        elif handle == 'z-scale' and base.bamboo.manipulationControl.useSeparateScaleHandles:
    ...
    def setScalingFactor(self, scaleFactor):
        ...
        self.scalingNode.setScale(
            self.ohScalingFactor * self.directScalingFactor)
    ...
    def growToFit(self):
        ...
        pos = base.bamboo.widget.getPos(base.bamboo.camera)
        minDim = min(base.bamboo.dr.nearWidth, base.bamboo.dr.nearHeight)
        sf = 0.15 * minDim * (pos[1]/base.bamboo.dr.near)
    ...
    def getAxisIntersectPt(self, axis):
        ...
        mCam2Widget = base.bamboo.camera.getMat(base.bamboo.widget)
        lineDir = Vec3(mCam2Widget.xformVec(base.bamboo.dr.nearVec))
        lineDir.normalize()
        # And determine where the viewpoint is relative to widget
        lineOrigin = VBase3(0)
    ...
    def getWidgetIntersectPt(self, nodePath, plane):
        ...
        mCam2NodePath = base.bamboo.camera.getMat(nodePath)
        ...
        lineDir = Vec3(mCam2NodePath.xformVec(base.bamboo.dr.nearVec))
        lineDir.normalize()
        # Find the hit point
        if plane == 'x':
