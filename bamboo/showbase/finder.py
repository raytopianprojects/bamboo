...
def replaceStateFunc(replaceFuncList):
    if not sys.modules.get('base.bamboo.fsm.State'):
        return
    from bamboo.fsm.State import State
    for oldFunc, funcName, newFunc in replaceFuncList:
...
def replaceIvalFunc(replaceFuncList):
    ...
    # a global ivalMgr.
    if not sys.modules.get('base.bamboo.interval.IntervalManager'):
        return
    from bamboo.interval.FunctionInterval import FunctionInterval
    for oldFunc, funcName, newFunc in replaceFuncList:
