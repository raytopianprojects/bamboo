...
if __debug__:
    ...
    def profileFunc(callback, name, terse, log=True):
        ...
        if log:
            ...
            _ProfileResultStr = '%s\n%s\n%s' % (
                prefix, _ProfileResultStr, suffix)
    ...
    def profiled(category=None, terse=False):
        ...
        """
        assert type(category) in (str, type(None)
...
def report(types=[], prefix='', xform=None, notifyFunc=None, dConfigParam=[]):
    ...
    globalClockDelta = importlib.import_module(
        "bamboo.distributed.ClockDelta").globalClockDelta
    ...
    def exceptionLogged(append=True):
        ...
        def _decoratorFunc(f, append=append):
            ...
            if exceptionLoggedNotify is None:
                from bamboo.directnotify.DirectNotifyGlobal import directNotify
                exceptionLoggedNotify = directNotify.newCategory(
                    "ExceptionLogged")
