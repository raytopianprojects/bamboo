...
class DirectJoybox(DirectObject):
    ...
    def __init__(self, device='CerealBox', nodePath=base.bamboo.camera,
                 headingNP=base.bamboo.camera):
        # See if device manager has been initialized
        if base.bamboo.deviceManager == None:
        ...
        self.device = device
        self.analogs = base.bamboo.deviceManager.createAnalogs(self.device)
        self.buttons = base.bamboo.deviceManager.createButtons(self.device)
        ...
        self.refCS = base.bamboo.cameraControl.coaMarker
        self.tempCS = base.bamboo.group.attachNewNode('JoyboxTempCS')
        # Text object to display current mode
        self.readout = OnscreenText.OnscreenText(
            pos=(-0.9, 0.95),
            font=base.bamboo.font,
            mayChange=1)
    ...
    def acceptUprightCameraEvent(self, button=L_UPPER):
        self.accept(self.getEventName(button),
                    base.bamboo.cameraControl.orbitUprightCam)
